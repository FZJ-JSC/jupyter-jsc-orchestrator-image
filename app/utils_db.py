...
def set_hostname_for_servername(app_logger, uuidcode, servername, database, value):
    app_logger.debug("uuidcode={} - Set hostname for userserver={} to {}".format(uuidcode, servername, value))
    with closing(psycopg2.connect(host=database.get('host'),
        ...
        with closing(con.cursor()) as cur: # auto closes
            with con: # auto commit
                cmd = "UPDATE server SET hostname = %s WHERE servername = %s"
...
def get_servername_tunnelup_for_user(app_logger, uuidcode, username, database):
    # return list of all servernames
    username = "{}%".format(username)
    with closing(psycopg2.connect(host=database.get('host'),
                                  port=database.get('port'),
...
def set_tunnelup(app_logger, uuidcode, servername, database, value):
    ...
    with closing(psycopg2.connect(host=database.get('host'),
        ...
        with closing(con.cursor()) as cur: # auto closes
            with con: # auto commit
                cmd = "UPDATE server SET tunnelup = %s WHERE servername = %s"
                app_logger.trace("uuidcode={} - Execute: {}, args: {}".format(uuidcode, cmd, (value, servername)))
                cur.execute(cmd,
                            (value, servername))

def get_tunnelup(app_logger, uuidcode, servername, database):
    app_logger.debug("uuidcode={} - Get tunnelup value for userserver={}".format(uuidcode, servername))
    infos = []
    with closing(psycopg2.connect(host=database.get('host'),
                                  port=database.get('port'),
                                  user=database.get('user'),
        ...
        with closing(con.cursor()) as cur: # auto closes
            with con: # auto commit
                cmd = "SELECT tunnelup FROM server WHERE servername = %s"
...
def create_entry_docker(app_logger, uuidcode, database, servername, jhubtoken, port, dockerimage):
    ...
    with closing(psycopg2.connect(host=database.get('host'),
        ...
        with closing(con.cursor()) as cur: # auto closes
            with con: # auto commit
                cmd = "INSERT INTO server (servername, jhubtoken, system, hostname, port, tunnelup, account, project, partition, reservation, Checkboxes, Resources, kernelurl, filesurl, spawning, skip, date) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, now())"
                app_logger.trace("uuidcode={} - Execute: {}".format(uuidcode, cmd))
